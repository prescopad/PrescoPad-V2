...
- **Key Features**:
  ...
  - Built-in self-pings keep-alive loop
...
Should return:
```json
{"success":true,"message":"PrescoPad API is running","version":"2.0.0"}
```
...
1. Start backend: `cd backend_python && .venv\Scripts\python main.py` (or use start-dev.bat)
...
1. Update `.env`:
   ...
   - Restrict CORS to specific domains/bundle IDs

2. Deploy to:
   - Render (Python hosting)
...
3. Update frontend config:
   ```typescript
   api: {
     baseUrl: 'https://prescopad-v2.onrender.com/api',
     timeout: 10000,
   }
   ```
...
- **API Routes**: See `backend_python/app/routes/`
- **Frontend Services**: See `frontend/src/services/`
- **Database Schema**: See index/collection definitions in `backend_python/app/config/database.py`
- **Local DB Schema**: `frontend/src/database/database.ts`
